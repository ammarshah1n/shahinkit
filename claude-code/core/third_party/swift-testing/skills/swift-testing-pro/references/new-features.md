...
- Verify each named API against the project's installed toolchain rather than guessing.
- If the installed toolchain does not support a feature, report it as unavailable and use no replacement unless the project already provides one.
...
Swift Testing can attach test output for failed tests. Do not add attachments by default. If the user explicitly requests them and the project permits them, attach only synthetic, non-sensitive data generated by the test; never attach user data, secrets, or local files.

Swift Testing supports `String`, `Data`, and values conforming to `Encodable`. Verify image support against the installed toolchain. Unlike the XCTest equivalent, attachments do not support lifetime controls.
