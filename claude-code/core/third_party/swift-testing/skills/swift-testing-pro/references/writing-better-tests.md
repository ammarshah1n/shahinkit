...
Strongly prefer to avoid hidden dependencies in production code you are testing. In Swift apps this commonly includes persistent settings, time, randomness, and networking. Suggest dependency injection; do not change production boundaries unless the user asks.

For networking, inject a narrow project-owned abstraction so tests can use an in-memory fake rather than a live service:
...
struct News {
    ...
    mutating func fetch(using loader: any DataLoading) async throws {
        let data = try await loader.data(for: url)
        stories = String(decoding: data, as: UTF8.self)
    }
}

struct DataLoaderMock: DataLoading {
    let response: Data
...
Keep tests on in-memory fakes, fixtures, and test-only URLs. Do not perform live networking or read or write a user's persistent settings. For existing persistent dependencies, suggest a project-owned protocol with an in-memory fake. Apply the same approach to clocks, randomness, and other shared state so meaningful tests remain isolated.
...
If you are writing tests related to a specific bug, use the `.bug` trait with the project bug ID when one exists. This extra data helps to provide context if the bug resurfaces.
...
Use a project-defined bug identifier when one is available:
