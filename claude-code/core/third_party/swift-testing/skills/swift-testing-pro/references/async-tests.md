...
Swift Testing is built to be async and run tests in parallel; take special care when Swift concurrency is involved. Use the project's installed toolchain and concurrency conventions as authoritative.


## Serializing tests

The `.serialized` trait disables parallel execution within the scope where it is applied. On a parameterized test, its cases run serially. On a suite, the suite's contained tests run serially relative to one another, including tests in nested suites. Applying it to a non-parameterized test function has no practical effect because that function has only one test case.

Use serialization only for tests that genuinely share mutable state. Prefer isolating that state so tests can remain parallel.
...
class ViewModel {
    func loadReadings(completion: @Sendable @escaping ([Double]) -> Void) {
        // Illustrative legacy callback boundary. Tests inject data in memory.
        completion([])
    }
}
```
