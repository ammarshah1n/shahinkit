...
FIXTURES = ROOT / "claude-code" / "core" / "tests" / "fixtures" / "b2" / "lifecycle"
...
class B2LifecycleTests(unittest.TestCase):
    ...
    def skill(self, name):
        return (ROOT / "claude-code" / "core" / "shared" / "skills" / name / "SKILL.md").read_text()
    ...
    def test_templates_define_each_authority(self):
        ...
        for template in self.contract["templates"]:
            content = (ROOT / "claude-code" / "core" / "shared" / "memory" / template).read_text()
    ...
    def test_portable_content_excludes_private_and_automatic_behavior(self):
        ...
        files.extend((ROOT / "claude-code" / "core" / "shared" / "memory" / name).read_text() for name in self.contract["templates"])
        corpus = "\n".join(files).lower()
