...
- `claude-code/core/third_party/ponytail/LOCK.json` and `claude-code/core/third_party/caveman/LOCK.json` contain repository URL, tag, commit, tree digest, per-file digest, license text/digest, vendoring date, and static-audit result.
...
- Claude Code core (`claude-code/core/shared/`) owns portable policy, skill bodies, memory templates, role schema, integration catalog, and render metadata.
...
`claude-code/core/integrations/dev-scope/` is optional, read-only stdio MCP using Node >=18, `@modelcontextprotocol/sdk ^1.29.0`, Zod `^3.25.0` or later, and `registerTool`.
...
`claude-code/core/scripts/manage.py` uses Python stdlib only. `.shahinkit-manifest.json` contains normalized relative owned paths, source/render digests, ownership, and adapter mappings only.
...
- `install` previews origin/provenance, host/scope, changed managed paths, active modes, trust requirement, host-reload requirement, receipt/backup impact. `--apply` mutates only after trust confirmation.
...
├── INSTALL.md
├── claude-code/core/docs/{PORTING-PLAN.md,integration-catalog.md}
├── claude-code/core/scripts/manage.py
├── claude-code/core/third_party/{ponytail,caveman}/{LICENSE,PROVENANCE.md,LOCK.json,skills/,tests/}
├── claude-code/core/shared/{policies/,models/{roles.schema.json,README.md},context/,skills/,memory/,mcp/}
...
├── claude-code/core/integrations/dev-scope/{README.md,package.json,package-lock.json,config.example.json,index.js,test/}
└── claude-code/core/tests/{test_manage.py,fixtures/hosts/{claude-code,opencode,codex}/{user,project}/}
```

`INSTALL.md` owns exact natural-language flow: canonical origin only; `--agent claude-code|opencode|codex`; preview; canonical GitHub owner/repository, release manifest, tag-signature, and maintainer-fingerprint verification; trust confirmation; `--apply`; host activation/reload check; receipt location; update/uninstall/rollback commands; public-release versus development-checkout rule; failure/restore acceptance. `--agent auto` is documented only with strict marker-set rules. No URL interception or implied remote execution.
...
- `sync --check` verifies generated drift. Static vendor audit precedes local vendor Claude Code core tests/rule-copy checks.
...
- Parse current host templates; test exact role renders and no worker inheritance; assert installer-managed active registrations after preview/apply/trust; assert documented host-reload behavior.
...
|---|---|---|
| Duplicated generic skills and memory | Merge | `claude-code/core/shared/`; adapters only render host mechanics. |
...
| Vendor portable assets | Pin/reconcile before render | `claude-code/core/third_party/` then Claude Code core shared/adapter output. |
