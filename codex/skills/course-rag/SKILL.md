...
Use this when the user wants to drag in course content, point Codex at a subject folder, or answer questions from local course files.
...
```bash
python3 codex/course-rag/scripts/build.py --subject "SUBJECT" --source "/path/to/course-folder"
```

The index is written under `codex/course-rag/indexes/` by default.

## Search

```bash
python3 codex/course-rag/scripts/search.py "SUBJECT" "query" --limit 8
```
