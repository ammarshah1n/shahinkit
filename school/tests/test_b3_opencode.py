...
CONTRACT = json.loads((ROOT / "school/tests/fixtures/b3/opencode/contract.json").read_text())
...
class B3OpenCodeTests(unittest.TestCase):
    ...
    def test_jsonc_example_uses_current_local_contract(self):
        ...
        self.assertEqual(config["plugin"], ["./.opencode/plugins/portable-gates.mjs"])
    ...
    def test_user_scope_example_uses_user_relative_paths(self):
        ...
        self.assertEqual(config["plugin"], ["./plugins/portable-gates.mjs"])
    ...
    def test_agents_are_explicit_independent_and_use_permissions_steps(self):
        for role, model in CONTRACT["roles"].items():
            ...
            self.assertNotIn("maxSteps", content)
