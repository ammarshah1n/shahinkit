...
FIXTURE = ROOT / "school" / "tests" / "fixtures" / "b3" / "claude-contract.json"
...
class B3ClaudeAdapterTests(unittest.TestCase):
    ...
    def test_json_assets_parse_and_use_current_claude_names(self):
        ...
        self.assertEqual(settings["source"], "claude-code/config/settings.patch.example.json")
        self.assertEqual(settings["mode"], "managed-block-merge")
    ...
    def test_pinned_vendor_skills_are_rendered_without_colliding_command_shims(self):
        ...
        for skill in ("ponytail", "ponytail-audit", "ponytail-debt", "ponytail-gain", "ponytail-help", "ponytail-review"):
            self.assertIn((f"school/third_party/ponytail/skills/{skill}/SKILL.md", f"skills/{skill}/SKILL.md"), copies)
        for skill in ("caveman", "caveman-commit", "caveman-help", "caveman-review"):
            self.assertIn((f"school/third_party/caveman/skills/{skill}/SKILL.md", f"skills/{skill}/SKILL.md"), copies)
    ...
    def test_agents_have_explicit_role_models_and_no_permission_bypass(self):
        roles = json.loads((ROOT / "school/shared/models/roles.schema.json").read_text())
        ...
        for name, model in self.contract["roles"].items():
            ...
            role = name.removeprefix("shahinkit-")
            self.assertEqual(defaults[role], model)
            self.assertEqual(rendered["roles"][role]["resolved_model"], model)
        controller = (ADAPTER / "agents/shahinkit-controller.md").read_text()
        self.assertIn("tools: Agent, Read, Glob, Grep, Bash, Edit, Write", controller)

    def test_lifecycle_state_is_explicit_and_generic_gates_stay_disabled(self):
        generic = json.loads((ADAPTER / "hooks/generic.disabled.example.json").read_text())
        self.assertEqual(generic["status"], "disabled-example-not-for-settings-merge")
        self.assertIn("SessionStart", generic["hooks"])
        self.assertIn("Stop", generic["hooks"])
        self.assertIn("SubagentStart", generic["hooks"])
    ...
    def test_security_and_failure_contracts(self):
        ...
        local = json.loads((ROOT / "school/shared/mcp/basic-memory/local-config.example.json").read_text())
        project = local["projects"]["<LOCAL_PROJECT_NAME>"]
        self.assertEqual(project["mode"], "local")
        self.assertIsNone(project["workspace_id"])
        self.assertIn(("school/shared/mcp/basic-memory/local-config.example.json", "config/basic-memory.local-config.example.json"), {(item["source"], item["destination"]) for item in self.render["copies"]})
        self.assertNotIn("API_KEY", json.dumps(server))
        corpus = "\n".join(
            path.read_text(errors="ignore")
            for path in ADAPTER.rglob("*")
            if path.is_file() and "course-rag" not in path.parts
        )
