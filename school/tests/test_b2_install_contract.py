...
class B2InstallContractTests(unittest.TestCase):
    ...
    def test_context_blocks_are_bounded_and_portable(self):
        blocks = {
            "school/shared/context/core-policy.block.md": "CORE-POLICY",
            "school/shared/context/ponytail-default.block.md": "PONYTAIL",
            "school/shared/context/caveman-default.block.md": "CAVEMAN",
            "school/shared/context/adapter-references.block.md": "ADAPTER-REFERENCES",
        }
        ...
        self.assertNotIn("school/shared/", rendered_destination)
        self.assertNotIn("school/third_party/", rendered_destination)
        caveman = self.read("school/shared/context/caveman-default.block.md")
    ...
    def test_basic_memory_templates_are_pinned_local_and_parseable(self):
        policy = self.load_json("school/shared/mcp/basic-memory/local-config.example.json")
        ...
        claude = json.loads(self.read("school/shared/mcp/basic-memory/claude-code.mcp.json").replace("{{BASIC_MEMORY_ENABLED}}", "false"))
        ...
        opencode = json.loads(self.read("school/shared/mcp/basic-memory/opencode.mcp.json").replace("{{BASIC_MEMORY_ENABLED}}", "false"))
        server = opencode["mcp"]["basic-memory"]
        self.assertEqual(server["type"], "local")
        self.assertFalse(server["enabled"])
        self.assertEqual(server["command"][0], "basic-memory")

        codex = tomllib.loads(self.read("school/shared/mcp/basic-memory/codex.mcp.toml").replace("{{BASIC_MEMORY_ENABLED}}", "false"))
    ...
    def test_optional_mcp_examples_are_disabled_and_secret_referential(self):
        examples = self.load_json("school/shared/mcp/optional/examples.json")["examples"]
        ...
        codex = tomllib.loads(self.read("school/shared/mcp/optional/codex.mcp.toml"))
        for server in codex["mcp_servers"].values():
            self.assertFalse(server["enabled"])

    def test_install_document_contract_and_fixtures(self):
        install = self.read("INSTALL.md")
        fixture_root = ROOT / "school/tests/fixtures/b2/install"
        ...
        required = (
            "school/scripts/manage.py` is Python-stdlib-only",
            CANONICAL_URL,
            "--agent claude-code",
            "--agent opencode",
            "--agent codex",
