...
class B3CodexTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.contract = json.loads((ROOT / "school/tests/fixtures/b3/codex-contract.json").read_text())
    ...
    def test_toml_templates_parse_and_use_separate_agent_files(self):
        ...
        fixture = json.loads((ROOT / "school/tests/fixtures/b3/codex-render-values.json").read_text())
    ...
    def test_generic_gates_are_disabled_and_no_unsupported_lifecycle_hook_is_rendered(self):
        generic = json.loads(self.read("hooks/generic-gates.disabled.example.json"))
        self.assertFalse(generic["enabled"])
        self.assertEqual(generic["kind"], "advisory")
        agents = self.read("AGENTS.md")
        self.assertIn("Caveman static context plus skills", agents)
        self.assertIn("No lifecycle hook is rendered", agents)
