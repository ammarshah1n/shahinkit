...
SHARED_SKILLS = sorted(path.parent.name for path in (ROOT / "school/shared/skills").glob("*/SKILL.md"))
MEMORY = sorted(path.name for path in (ROOT / "school/shared/memory").glob("*.md"))
...
class B3CrossAdapterTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.schema = json.loads((ROOT / "school/shared/models/roles.schema.json").read_text())
    ...
    def test_context_and_skill_maps_reference_canonical_assets(self):
        ...
        for skill in SHARED_SKILLS:
            self.assertRegex((ROOT / "school/shared/skills" / skill / "SKILL.md").read_text(), rf"(?m)^name: {re.escape(skill)}$")
    ...
    def test_context_renders_match_canonical_bytes_and_render_sources_are_digest_backed(self):
        ...
        for host, render in adapters:
            ...
            if "settings_patches" in render:
                sources.add(render["settings_patches"]["source"])
            ...
            sources.update(f"school/shared/skills/{skill}/SKILL.md" for skill in skills)
            sources.update(f"school/shared/memory/{name}" for name in memory)
            for vendor, names in render.get("vendor_skills", {}).items():
                sources.update(f"school/third_party/{vendor}/skills/{name}/SKILL.md" for name in names)
            for source in sources:
                candidate = assert_safe_manifest_path(self, source)
                self.assertTrue(candidate.is_file(), source)
                if source.startswith(f"{host}/"):
                    relative = source.removeprefix(f"{host}/")
                    self.assertIn(relative, manifest["adapter_inputs"][host])
                    self.assertEqual(manifest["adapter_inputs"][host][relative], hashlib.sha256(candidate.read_bytes()).hexdigest())
                else:
    ...
    def test_required_install_assets_are_explicit_and_present(self):
        expected = {
            "claude-code": ["CLAUDE.md", "config/mcp.basic-memory.example.json", "agents/shahinkit-controller.md", "skills/course-rag/SKILL.md", "course-rag/scripts/build.py", "course-rag/scripts/search.py"],
            ...
            "opencode": ["AGENTS.md", "opencode.jsonc.example", "opencode.user.jsonc.example", ".opencode/plugins/portable-gates.mjs", ".opencode/plugins/portable-gates.features.mjs", ".opencode/agents/controller.md"],
        }
    ...
    def test_exact_role_defaults_and_no_inheritance(self):
        claude_roles = json.loads((ROOT / "school/tests/fixtures/b3/claude-contract.json").read_text())["roles"]
        codex_roles = json.loads((ROOT / "school/tests/fixtures/b3/codex-contract.json").read_text())["roles"]
        opencode_roles = json.loads((ROOT / "school/tests/fixtures/b3/opencode/contract.json").read_text())["roles"]
        expected_defaults = {
            "claude-code": {name.removeprefix("shahinkit-"): model for name, model in claude_roles.items()},
            "codex": codex_roles,
        ...
        for role in ROLES:
            claude = (ROOT / "claude-code/agents" / f"shahinkit-{role}.md").read_text()
            self.assertRegex(claude, rf"(?m)^name: shahinkit-{role}$")
            self.assertIn(f"model: {claude_roles[f'shahinkit-{role}']}", claude)
    ...
    def test_exact_host_destination_maps_and_activation_claims(self):
        ...
        self.assertIn("./.opencode/plugins/portable-gates.mjs", json.loads((ROOT / "opencode/opencode.jsonc.example").read_text())["plugin"])

    def test_codex_scope_destinations_join_fixture_roots(self):
        fixtures = ROOT / "school/tests/fixtures/hosts/codex"
        home = fixtures / "home"
        expected = {
            "user": {
        ...
        for scope, spec in render["scope_templates"].items():
            for ponytail, caveman in (("true", "true"), ("false", "true"), ("true", "false"), ("false", "false")):
                ...
                with tempfile.TemporaryDirectory() as temporary:
                    ...
                    plugin = output / config["plugin"][0].removeprefix("./")
...
if (output.system.length !== {expected}) process.exit(1);'''], text=True, capture_output=True, check=False)
    ...
    def test_render_manifest_defaults_enable_supported_features(self):
        expected = {
            "claude-code": {"required": {"SHAHINKIT_DATA_DIR", "SHAHINKIT_STATE_DIR", "LOCAL_BASIC_MEMORY_CONFIG_DIR", "LOCAL_PROJECT_NAME", "LOCAL_PROJECT_PATH", "LOCAL_PROJECT_ROOT", "PONYTAIL_ENABLED", "CAVEMAN_ENABLED"}, "defaults": {}},
            "codex": {"required": {"SHAHINKIT_DATA_DIR", "SHAHINKIT_STATE_DIR", "LOCAL_BASIC_MEMORY_CONFIG_DIR", "LOCAL_PROJECT_NAME", "LOCAL_PROJECT_PATH", "LOCAL_PROJECT_ROOT", "PONYTAIL_ENABLED", "CAVEMAN_ENABLED"}, "defaults": {}},
    ...
    def test_generic_gates_disabled_basic_memory_local_and_no_unsupported_fields(self):
        ...
        self.assertEqual(claude_generic["status"], "disabled-example-not-for-settings-merge")
        self.assertFalse(codex_generic["enabled"])
        local_config = json.loads((ROOT / "school/shared/mcp/basic-memory/local-config.example.json").read_text())
        ...
        shared_claude = json.loads((ROOT / "school/shared/mcp/basic-memory/claude-code.mcp.json").read_text())["mcpServers"]["basic-memory"]
        shared_codex = tomllib.loads((ROOT / "school/shared/mcp/basic-memory/codex.mcp.toml").read_text())["mcp_servers"]["basic-memory"]
        shared_opencode = json.loads((ROOT / "school/shared/mcp/basic-memory/opencode.mcp.json").read_text())["mcp"]["basic-memory"]
    ...
    def test_manifest_covers_each_adapter_input_with_fresh_digest_and_no_symlinks(self):
        ...
        for host in ("claude-code", "codex", "opencode"):
            actual = {
                ...
                for path in (ROOT / host).rglob("*") if path.is_file()
            }
